...
- Fixed the code app chat loop where the hosted flow could return a stale `nextQuestion` such as `Which app, process, table, report, or data source is affected?` after the local draft had already captured the affected app. The code app now rejects stale or duplicate follow-up questions and advances to the next locally missing requirement. Published the fix to `SPDEV-Dev2` / `WorkManagementAgent` on 2026-06-10; the refreshed solution export shows code app package asset `index-DOf-EqVs.js`.
